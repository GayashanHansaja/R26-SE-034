...
| A-D2 | **Store tokens**: `api_tokens` SQLite table — `id, name, sha256(token), scopes, created_at, expires_at (nullable), revoked_at (nullable)`. Value = 32 bytes `crypto/rand` → `erpbt_` + 64 hex, **returned once** at creation. Lookup by `sha256(bearer)` per request. | Hash-only storage (DB leak-safe); kubectl `create token` value-once behavior. |
...
| A-D8 | **CORS**: `CORS_ALLOWED_ORIGINS` env (comma list), default `*`; wired into `WithCORSAllowedOrigins` (server.go:469-473). | Back-compat; docs recommend explicit origins in production. |
| A-D9 | **bridgectl**: `Context` gains `api-token` (yaml `api-token`, env `BRIDGE_API_TOKEN`, persistent `--token` flag); one shared `newRequest(ctx, method, url, body, token)` helper injects `Authorization: Bearer`; all 11 call sites + new token commands route through it. | Mirrors kubeconfig `users[].user.token`; single seam. |
...
**In:** `api_tokens` store + validation middleware; token CRUD routes; MCP/metrics/logs gating; bridgectl `token create/list/revoke` + `api-token` plumbing; CORS origins env; docs + Postman + CHANGELOG.

**Out:** OAuth AS / PKCE / dynamic client registration; per-tool/module scoping; stdio auth; token refresh/rotation UI; rate-limit-per-token (MCP per-session buckets already cover it); multi-user identities.
...
- [ ] **A1 — Token store** — `api_tokens` table + CRUD.
      ...
      **Verify:** `go test ./internal/mcp/ -run TestStore`)
      Schema: `api_tokens(id TEXT PK, name TEXT, token_hash TEXT UNIQUE, scopes TEXT, created_at INTEGER, expires_at INTEGER NULL, revoked_at INTEGER NULL)`.
...
- [ ] **A3 — Auth middleware** — enforcement + admin compare + scope gate.
      ...
      **Verify:** env unset ⇒ open + warning logged; env set ⇒ no header 401, env token 200, store token 200, wrong scope 403, revoked 401, expired 401)
      `authorize(r) (role string, ok bool)`: constant-time compare against env token → "admin"; else sha256 lookup in store → check revoked/expiry/scope.
- [ ] **A4 — MCP bearer gate** — `/mcp/` requires token when enforcement on.
      ...
      **Verify:** `curl -X POST http://localhost:8080/mcp/` without token ⇒ 401 when `API_AUTH_TOKEN` set; with `mcp`-scoped token ⇒ proceeds to MCP handshake)
      Note: the existing SSE tools/list filter (server.go:477-549) must sit inside the gate so 401s are not filtered.
...
- [ ] **A7 — bridgectl token commands** — `token create --name <n> [--scope mcp,metrics,logs] [--expires 30d]`, `token list`, `token revoke <id>`.
      ...
      **Verify:** `go test ./internal/cli/ -run TestToken`; create prints value once; revoke prompts like `tool delete` (confirm unless `--yes`))
      `--expires` accepts Go durations + `d` suffix (`30d` → 720h) — parse in one helper.
- [ ] **A8 — bridgectl api-token plumbing** — context field + shared request helper + all call sites.
...
- [ ] **A9 — CORS origins** — `CORS_ALLOWED_ORIGINS` env.
      (**Seam:** server.go:469-473; **Files:** internal/mcp/server.go, main.go, docs/environment-variables.md;
      **Verify:** `go test ./internal/mcp/`; manual `Origin` header reflects config; default `*` when unset)
- [ ] **A10 — Docs + Postman + CHANGELOG**.
      (**Files:** docs/tokens.md (new — workflow: set env token → `token create` → frontend MCP bearer / Prometheus `bearer_token` → revoke), docs/api.md (token endpoints + auth header), docs/environment-variables.md (`API_AUTH_TOKEN`, `CORS_ALLOWED_ORIGINS`), docs/connectivity.md (MCP bearer), erpbridge_postman_collection.json (token create + authenticated invoke), CHANGELOG.md (Unreleased);
      **Verify:** `rg -n "API_AUTH_TOKEN" docs/` covers every new var; Postman JSON parses)
...
4. **Expiry**: `--expires` in the past ⇒ 401.
...
7. **CLI hygiene**: every bridgectl request goes through `newRequest()`; no bare `http.Get`/`DefaultClient` remains.

## Open Questions

None — all settled during the design tree (grilling) session. Decisions A-D1..A-D11 are confirmed; implementation order: A1 → A2 → A3 → A4/A5 → A6 → A7 → A8 → A9 → A10.
...
- **A8 is the highest-risk task** — touches 9 call sites. The `newRequest()` helper should be implemented + tested before converting any call sites.
- **A8 regression gate**: The verify command (`rg -n "http.Get|DefaultClient" internal/cli/`) is excellent — consider adding it as a CI check.
- **Dependency with Plan-main**: A8's `newRequest()` helper should land before Plan-main D1–D3 (CLI improvements), since both touch the same call sites. Run S1–S3 from Plan-main before A1 (they close real security vulnerabilities with minimal risk and zero dependency on auth).
